...
if [ "$JAVA_MODULE_SYSTEM_ENABLED" == "true" ]; then
  ...
  java $JPMS_SWITCHES -p /app/lib -m no.ssb.dapla.data.access/no.ssb.dapla.data.access.DataAccessApplication
else
  echo "Starting java using CLASSPATH"
  java -cp "/app/lib/*" no.ssb.dapla.data.access.DataAccessApplication
fi
